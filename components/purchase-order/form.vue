<template>
  <el-dialog
    v-model="show"
    width="1100px"
    :title="!!form?.id ? 'EDIT PURCHASE ORDER' : 'CREATE NEW PURCHASE ORDER'"
  ...
  >
    <el-form label-width="160px" label-position="left">
      ...
      <el-card shadow="never" class="mb-4">
        <template #header>
          <span class="font-semibold">PURCHASE ORDER INFORMATION</span>
        </template>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            ...
            <el-form-item label="Order Type" :error="errors.orderType">
              <el-select
      ...
      <el-card shadow="never" class="mb-4">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="Warehouse" :error="errors.warehouse">
              <el-input placeholder="Warehouse" v-model="form.warehouse" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item
              label="Packing Condition"
              :error="errors.packingCondition"
            >
              <el-input
                placeholder="Packing condition"
                v-model="form.packingCondition"
              />
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <template #header>
          <div class="flex items-center justify-between">
            <span class="font-semibold">
              PURCHASE ORDER ITEMS ({{ form.items.length }})
            </span>
            <div>
              <el-button
                type="success"
...
function setMaterial(partNumber, item) {
  ...
  if (material) {
    item.description = [material.name, material.model]
      .filter(Boolean)
      .join(" - ");
    item.unitPrice = material.purchasePrice;
    calculateTotals();
  }
}
...
const openForm = (data = {}) => {
  form.value = {
    ...
    orderType: data.orderType || "Stock Order",
...
const save = async () => {
  try {
    ...
    const url = form.value.id
      ? `/api/purchase-orders/${form.value.id}`
      : "/api/purchase-orders";
